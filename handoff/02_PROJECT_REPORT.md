...
Each direction below is motivated by a specific measurement in this report rather than
by generality, and they are ordered by expected impact.

**Annotating a small spine subset is the highest-value next step.** Naming or numbering a
vertebra is supervised by nature, and §4.5 quantifies what that costs us: our
annotation-free network reaches Dice 0.380 against the reference's published 0.920 and
produces zero numbered vertebrae. Twenty to thirty annotated slices would allow a
supervised head on the features the self-supervised network already learns, which changes
what the system can output rather than merely improving a score.

**A larger cohort is the honest requirement behind our spine conclusions.** Canal width
separates the groups at p = 0.089 and the per-level minimum at p = 0.141; both point in
the direction stenosis predicts and neither reaches significance. This is a sample-size
limitation at 10 versus 9 patients, not a methodological one, and roughly forty per group
would resolve it.

**Pathology-specific measurements should replace a single canal metric.** The per-level
analysis reaches AUC 0.688 partly because the pathological cohort is not uniformly
stenotic: disc herniation and degeneration do not narrow the canal, so no canal-derived
statistic can detect them. Disc height, protrusion and signal intensity would address the
conditions the present measure cannot see.

**Volumetric segmentation** would exploit through-plane context that slice-wise
processing discards; vertebrae are three-dimensional structures. This is currently
blocked by the 6 GB device against the 16 GB+ requirement documented for 3D BraTS models,
not by the method.

Lower-risk refinements follow: distilling the pretrained model's output into pseudo-labels
to remove the inference-time dependency; ensembling and test-time augmentation on brain,
typically worth one to two Dice points; a boundary-aware loss term, indicated by Hausdorff
being our weakest metric relative to Dice; and deterministic kernel selection so the
self-supervised network can be reported as a single reproducible figure rather than a mean
over three runs. Clinical deployment would additionally require axial and coronal support,
DICOM input, and a radiologist reader study, since no metric substitutes for confirmation
that the output is clinically useful.
